...
export function Dashboard() {
  ...
  const expiringSoon = products
    ...
    .filter(product => {
      ...
      return days <= settings.brigadeAutoSuggest;
    })
  ...
  return (
    <div className="space-y-8 flex flex-col">
      ...
      {searchResults.length > 0 && (
        <div className="bg-white rounded-[32px] border-2 border-slate-100 shadow-sm p-4 space-y-2">
          ...
          {searchResults.map(product => {
            const days = getDaysUntil(product.expirationDate);
            return (
              <div key={product.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-[20px]">
                <div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <StatusCard title="Crítico" subtitle={`até ${settings.alertCritical} dias`} count={criticalCount} color="orange" />
        <StatusCard title="Atenção" subtitle={`até ${settings.alertMedium} dias`} count={attentionCount} color="yellow" />
        <StatusCard title="Saudável" subtitle={`+${settings.alertMedium} dias`} count={healthyCount} color="green" />
      </div>
...
function StatusCard({ title, subtitle, count, color }: { title: string, subtitle?: string, count: number, color: 'red' | 'orange' | 'yellow' | 'green' }) {
  const colorMap = {
