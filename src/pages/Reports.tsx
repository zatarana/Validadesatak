...
export function Reports() {
  ...
  const monthlyVariation = previousItemsDiscarded > 0 ? Math.round(((totalItemsDiscarded - previousItemsDiscarded) / previousItemsDiscarded) * 100) : null;
  ...
  return (
    <div className="space-y-6 pb-2 w-full overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <StatCard title="Mês Anterior" value={previousItemsDiscarded} color="slate" subtitle={monthlyVariation === null ? 'sem base' : `${monthlyVariation > 0 ? '+' : ''}${monthlyVariation}%`} />
...
function StatCard({ title, value, color, subtitle }: { title: string, value: number, color: string, subtitle?: string }) {
  ...
  return <div className={`p-6 rounded-[32px] shadow-sm border-2 flex flex-col justify-between ${colorMap[color]}`}><p className="text-[10px] font-black uppercase tracking-[0.1em] mb-2 opacity-80">{title}</p>{subtitle && <p className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-2">{subtitle}</p>}<span className="text-4xl font-black tracking-tighter leading-none">{new Intl.NumberFormat('pt-BR').format(value)}</span></div>;
}
