...
import { exportDiscardsCsv, printProductsReport, shareWhatsAppSummary } from '../lib/export';
  ...
  return (
    <div className="space-y-6 pb-2 w-full overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button onClick={() => shareWhatsAppSummary(products, discardRecords, settings)} className="bg-white border-2 border-emerald-100 rounded-2xl p-4 flex items-center justify-center gap-2 font-black text-[10px] uppercase tracking-widest text-emerald-600 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2 bg-slate-900 text-white p-6 rounded-[32px] shadow-sm flex items-center justify-between">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-slate-300 font-bold mt-1">{dominantCategory ? `Categoria com maior perda: ${dominantCategory.name}` : 'Registre descartes para gerar análise.'}</p>
          </div>
        </div>
      </div>

      <div className="bg-emerald-50 rounded-[40px] p-6 border-2 border-emerald-100 flex items-center gap-6 shadow-sm">
        <div className="w-16 h-16 rounded-full flex items-center justify-center bg-white shadow-md border-4 border-emerald-100 shrink-0"><Lightbulb className="text-emerald-500" size={24} strokeWidth={2.5} /></div>
        <div className="flex flex-col">
          <h4 className="text-emerald-900 font-black text-lg tracking-tight mb-1">Dica Inteligente</h4>
          <p className="text-sm font-bold text-emerald-800/80 leading-snug">{dominantCategory ? `${dominantCategory.name} lidera os descartes. Confira compra, exposição e giro desse setor.` : 'Quando houver descartes, esta área mostrará onde agir primeiro.'}</p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-[32px] shadow-sm border-2 border-slate-100 flex flex-col items-center">
        ...
        {pieData.length > 0 ? <><div className="h-56 w-full flex justify-center items-center"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={pieData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={4} dataKey="value" stroke="none">{pieData.map((entry, index) => <Cell key={`cell-${entry.name}`} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip formatter={(value: number) => new Intl.NumberFormat('pt-BR').format(value)} /></PieChart></ResponsiveContainer></div><div className="flex justify-center gap-6 w-full mt-4 flex-wrap">{pieData.map((item, index) => <div key={item.name} className="flex flex-col items-center gap-1 text-[10px] font-black uppercase tracking-widest text-slate-500 shrink-0"><div className="w-4 h-4 rounded-full" style={{ backgroundColor: COLORS[index % COLORS.length] }} />{item.name}</div>)}</div></> : <div className="text-center py-10 text-slate-500 font-bold bg-slate-50 rounded-[24px] w-full">Nenhum descarte registrado ainda.</div>}
      </div>

      <div className="bg-white p-6 rounded-[32px] shadow-sm border-2 border-slate-100">
        <h3 className="text-xl font-black text-slate-900 tracking-tight mb-6">Top produtos descartados</h3>
        {topProducts.length > 0 ? <div className="space-y-3">{topProducts.map(item => <div key={item.name} className="flex justify-between gap-4 text-slate-900"><span className="font-bold truncate">{item.name}</span><span className="font-black text-red-600 shrink-0">{item.quantity} un</span></div>)}</div> : <div className="text-center py-8 text-slate-500 font-bold bg-slate-50 rounded-[24px]">Sem produtos descartados.</div>}
      </div>

      <div className="bg-white p-6 rounded-[32px] shadow-sm border-2 border-slate-100">
        <h3 className="text-xl font-black text-slate-900 tracking-tight mb-6 flex items-center gap-2"><CalendarDays size={20} /> Itens adicionados por data</h3>
        <input type="date" value={addedDate} onChange={event => setAddedDate(event.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-[20px] px-5 py-4 font-bold text-slate-800 focus:outline-none focus:ring-4 focus:ring-indigo-100" />
        <div className="mt-4 space-y-3">
          {addedDate ? addedProducts.slice(0, 8).map(product => <div key={product.id} className="flex justify-between items-center border-b border-slate-100 pb-3 last:border-0"><div><p className="font-black text-slate-800">{product.name}</p><p className="text-xs font-bold text-slate-400">{product.brand || 'Sem marca'} • vence {formatPtDate(product.expirationDate)}</p></div><span className="text-xs font-black text-indigo-600">{product.quantity ?? 1} un</span></div>) : <p className="text-slate-500 font-bold py-4">Selecione uma data para ver os itens adicionados.</p>}
          {addedDate && addedProducts.length === 0 && <p className="text-slate-500 font-bold py-4">Nenhum item adicionado nesta data.</p>}
        </div>
      </div>

      <div className="bg-white p-6 rounded-[32px] shadow-sm border-2 border-slate-100 mb-8">
        <h3 className="text-xl font-black text-slate-900 tracking-tight mb-4">Histórico de descartes por data <span className="text-slate-400">({filteredDiscards.length})</span></h3>
        <div className="space-y-3 mb-4">
          <input type="date" value={discardDate} onChange={event => setDiscardDate(event.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-[20px] px-5 py-4 font-bold text-slate-800 focus:outline-none focus:ring-4 focus:ring-indigo-100" />
          <div className="relative"><Search size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" /><input value={discardSearch} onChange={event => setDiscardSearch(event.target.value)} placeholder="Buscar por nome, marca ou código" className="w-full bg-slate-50 border-2 border-slate-100 rounded-[20px] pl-12 pr-5 py-4 font-bold text-slate-800 focus:outline-none focus:ring-4 focus:ring-indigo-100" /></div>
        </div>
        {filteredDiscards.length > 0 ? <div className="space-y-4">{filteredDiscards.slice(0, 20).map(record => <div key={record.id} className="flex justify-between items-center border-b-2 border-slate-50 pb-4 last:border-0 last:pb-0"><div className="flex flex-col"><span className="font-black text-slate-800">{record.productName}</span><span className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">{new Date(record.discardedAt).toLocaleString('pt-BR')} • {record.reason}</span><span className="text-xs font-bold text-slate-400 mt-1">{record.productBrand} • Cód: {record.productBarcode || '-'}</span></div><div className="bg-rose-50 text-rose-600 px-3 py-1 rounded-xl font-black text-sm">-{record.quantity}</div></div>)}</div> : <div className="text-center py-8 text-slate-500 font-bold bg-slate-50 rounded-[24px]">Nenhum descarte encontrado.</div>}
      </div>
    </div>
  );
}
